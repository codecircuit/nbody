...
int main(int argc, char* argv[]) {
	...
	CUcontext context;
	CUmodule module;
	CUfunction up_kernel; // update positions kernel
	CUfunction us_kernel; // update speed kernel
	CUresult err;

	auto myCheck = [] (CUresult err) {
		if (err != CUDA_SUCCESS) {
			const char* msg;
	...
	cout << "  - Initializing CUDA + platform..." << flush;
	myCheck(cuInit(0));

	cout << "  - Get CUDA Device..." << flush;
	myCheck(cuDeviceGet(&device, 0));

	cout << "  - Get device name..." << flush;
	char name[256];
	myCheck(cuDeviceGetName(name, 256, device));
	cout << "    Device name: " << name << endl;

	cout << "  - Get Device Compute Capability..." << flush;
	int major = 0, minor = 0;
	myCheck(cuDeviceComputeCapability(&major, &minor, device));
	cout << "    Architecture: sm_" << major << minor << endl;

	// kernel names
	const char* up_kernel_name = "updatePositions";
	const char* us_kernel_name = "updateSpeed";

	cout << "  - Initializing CUDA Context..." << flush;
	myCheck(cuCtxCreate(&context, 0, device));

	cout << "  - Loading module " << fkernel << "..." << flush;
	myCheck(cuModuleLoad(&module, fkernel.c_str()));

	cout << "  - Acquiring kernel function " << up_kernel_name << "..." << flush;
	myCheck(cuModuleGetFunction(&up_kernel, module, up_kernel_name));
	cout << "  - Acquiring kernel function " << us_kernel_name << "..." << flush;
	myCheck(cuModuleGetFunction(&us_kernel, module, us_kernel_name));
	...
	CUdeviceptr vel_z_d;
	CUdeviceptr m_d;
	CUdeviceptr* all_d[] = { &pos_x_d, &pos_y_d, &pos_z_d, &vel_x_d, &vel_y_d, &vel_z_d, &m_d };
	for (int i = 0; i < 7; ++i) {
		if (cuMemAlloc(all_d[i], size) != CUDA_SUCCESS) {
			cout << "[FAILED]" << endl;
			exit(1);
		}
	}
	...
	for (int i = 0; i < 7; ++i) {
		if (cuMemcpyHtoD(*all_d[i], *all_h[i], size) != CUDA_SUCCESS) {
			cout << "[FAILED]" << endl;
			exit(1);
		}
	}
	...
	void* args_us[] = { &m_d, &pos_x_d, &pos_y_d, &pos_z_d, &vel_x_d, &vel_y_d, &vel_z_d, &dt, &e, &N };
	void* args_up[] = { &pos_x_d, &pos_y_d, &pos_z_d, &vel_x_d, &vel_y_d, &vel_z_d, &dt, &N };
	...
	// KERNEL LAUNCH //
	auto checkCudaResult = [] (CUresult err) {
		if (err != CUDA_SUCCESS) {
			const char* msg;
			cuGetErrorName(err, &msg);
			cout << "[FAILED] " << msg << endl;
	...
	for (unsigned i = 0; i < T; ++i) {
		if (!fnosu) {
			checkCudaResult(cuLaunchKernel(us_kernel,          // update speed kernel
		...
		if (!fnopu) {
			checkCudaResult(cuLaunchKernel(up_kernel,          // update positions kernel
			                               blocks, 1, 1,       // Grid Size
			                               threads, 1, 1,      // Block Size
			                               0, 0, args_up, 0));
			checkCudaResult(cuCtxSynchronize());
		...
		if (!fout.empty() && !checkResult && !writeOnlyLast) { // write gpu result to file
			for (int i = 0; i < 6; ++i) { // we skip copying the masses
				if (cuMemcpyDtoH(*all_h[i], *all_d[i], size) != CUDA_SUCCESS) {
	...
	if (checkResult) {
		for (int i = 0; i < 6; ++i) { // we skip copying the masses
			if (cuMemcpyDtoH(*all_gpures[i], *all_d[i], size) != CUDA_SUCCESS) {
				cout << "[FAILED]" << endl;
				exit(1);
			}
		}
	...
	for (int i = 0; i < 7; ++i) {
		if (cuMemFree(*all_d[i]) != CUDA_SUCCESS) {
			cout << "[FAILED]" << endl;
			exit(1);
		}
	}
	cout << "[OK]" << endl;
	cout << "  - Going to free cuda context..." << flush;
